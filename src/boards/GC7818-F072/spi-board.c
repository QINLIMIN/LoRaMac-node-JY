...
void SpiInit( Spi_t *obj, SpiId_t spiId, PinNames mosi, PinNames miso, PinNames sclk, PinNames nss )
{
    ...
    if( nss == NC )
    {
        ...
        SpiFormat( obj, 0, SPI_POLARITY_LOW, SPI_PHASE_1EDGE, 0 );
    }
    else
    {
        SpiFormat( obj, 1, SPI_POLARITY_LOW, SPI_PHASE_1EDGE, 1 );
    }
...
uint16_t SpiInOut( Spi_t *obj, uint16_t outData )
{
    ...
    *(__IO uint8_t *)(&(SpiHandle[obj->SpiId].Instance->DR)) = ( uint16_t ) ( outData & 0xFF );  
...
int GetSpiCR1(void)
{
    return SpiHandle[1].Instance->CR1;
}
int GetSpiCR2(void)
{ 
    return SpiHandle[1].Instance->CR2; 
}
int I2sCFGR(void)
{
    return SpiHandle[1].Instance->I2SCFGR;
}
int GetSpiCRCPR(void)
{
   return SpiHandle[1].Instance->CRCPR;
}
 //printf("Lora frequency = %dHz\n", GetLoraFrequency());//wzh 
/////////////wzh end///////// 
