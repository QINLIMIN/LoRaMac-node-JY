...
void BoardInitMcu( void )
{
    if( McuInitialized == false )
    {
        ...
        GpioInit( &LedOnBoard, LED_ON_BOARD, PIN_OUTPUT, PIN_PUSH_PULL, PIN_NO_PULL, 1 );
    ...
    SX1272IoInit( );
#elif defined( SX1276MB1LAS ) || defined( SX1276MB1MAS )
    SpiInit( &SX1276.Spi, SPI_2/*SPI_1*/, RADIO_MOSI, RADIO_MISO, RADIO_SCLK, NC );
    SX1276IoInit( );
#endif
...
static void BoardUnusedIoInit( void )
{
    //HAL_DBGMCU_EnableDBGSleepMode( );  //wzh
...
void SystemClockConfig( void )
{
    ...
    RCC_OscInitStruct.PLL.PLLMUL = RCC_PLL_MUL6;//RCC_PLL_MUL2;
    ...
    SystemCoreClockUpdate();//wzh
//GpioToggle(&LedOnBoard); //wzh down
    PeriphClkInit.PeriphClockSelection = RCC_PERIPHCLK_RTC /*| RCC_PERIPHCLK_USART1*/;
    PeriphClkInit.RTCClockSelection = RCC_RTCCLKSOURCE_HSE_DIV32;//RCC_RTCCLKSOURCE_LSI;
...
void SystemClockReConfig( void )
{
    ...
    // Enable HSI
    __HAL_RCC_HSI_ENABLE();//_HAL_RCC_HSI_CONFIG( RCC_HSI_ON );
...
{
    UartPutBuffer( &Uart1, ( uint8_t* )data, ( uint16_t )strlen(data) );
}
// Jimmy end
